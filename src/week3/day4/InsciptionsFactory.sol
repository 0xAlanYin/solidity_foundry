...
contract InsciptionsProxyFactory {
    ...
    event InsciptionsProxyFactory_MintInscription(address indexed tokenAddr, address indexed sender, uint256 perMint);
    ...
    constructor() {
        implementation = address(new InsciptionsERC20());
        ...
        returns (address)
    {
        ...
        InsciptionsERC20(cloned).initialize(address(this), _symbol, _totalSupply, _perMint);
    ...
    function mintInscription(address tokenAddr) public payable {
        ...
        address minter = msg.sender; // save gas
        // mint token
        InsciptionsERC20(tokenAddr).mint(minter);

        // 收取手续费
        Address.sendValue(owner, totalCost * FIX_FEE / 100);

        // 向合约部署者转账，收取 mint 费用
        Address.sendValue(payable(inscription.deployer), totalCost * (100 - FIX_FEE) / 100);

        emit InsciptionsProxyFactory_MintInscription(tokenAddr, minter, inscription.perMint);
    }
